...
def test_resolve_workload_base_only(tmp_path):
    ...
    workload_file.write_text(yaml.dump({
        "framework": "megatron",
        "config": {
...
def test_resolve_workload_inheritance(tmp_path):
    ...
    (templates_dir / "child.yaml").write_text(yaml.dump({
        "from": "base",
        "config": {
...
def test_resolve_workload_circular_detection(tmp_path):
    ...
    (templates_dir / "a.yaml").write_text(yaml.dump({
        "framework": "megatron",
        "from": "b.yaml",
        "config": {"tensor-model-parallel-size": 1},
    }))
    (templates_dir / "b.yaml").write_text(yaml.dump({
        "framework": "megatron",
        "from": "a.yaml",
        "config": {"tensor-model-parallel-size": 2},
    }))
...
def test_list_traces_pagination(tmp_path):
    ...
    for i in range(5):
        ...
        trace_dir.mkdir()
        (trace_dir / "workload.yaml").write_text(yaml.dump({
            "framework": "megatron",
            "from": "base",
            "config": {"tensor-model-parallel-size": 1},
        }))
...
def test_list_traces_gpu_filter(tmp_path):
    ...
    h100_trace.mkdir()
    (h100_trace / "workload.yaml").write_text(yaml.dump({
        "framework": "megatron",
        "config": {"tensor-model-parallel-size": 4},
    }))

    a100_trace = gpu_dir / "a100" / "traces" / "hash_a100"
    a100_trace.mkdir()
    (a100_trace / "workload.yaml").write_text(yaml.dump({
        "framework": "megatron",
        "config": {"tensor-model-parallel-size": 8},
    }))
