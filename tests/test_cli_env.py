"""Tests for cascading .tracking.env loading in simulon CLI."""
...
from simulon.cli.simulate import _load_cascading_tracking_env, _load_tracking_env_file


class TestLoadTrackingEnvFile:
    def test_empty_file(self, tmp_path: Path) -> None:
        env_file = tmp_path / ".tracking.env"
        env_file.write_text("")
...
class TestLoadCascadingTrackingEnv:
    def test_loads_all_levels_in_order(
        ...
        (cwd / ".tracking.env").write_text("A=root\nB=root")
        (level1 / ".tracking.env").write_text("A=level1")
        (level2 / ".tracking.env").write_text("C=level2")
        (level3 / ".tracking.env").write_text("B=level3")

        _load_cascading_tracking_env(str(scenario))
    ...
    def test_later_level_can_override(
        ...
        """The later file in the cascade CAN override an earlier .tracking.env-only value."""
        monkeypatch.chdir(tmp_path)
        cwd = tmp_path
        level1 = cwd / "exp"
        level1.mkdir()
    ...
    def test_empty_dirs_no_files(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        _load_cascading_tracking_env(str(dir1 / "scenario.yaml"))
    ...
    def test_scenario_below_cwd(self, tmp_path: Path) -> None:
        ...
        scenario_dir.mkdir()
        (scenario_dir / ".tracking.env").write_text("SCENARIO_ONLY=yes")
        scenario_file = scenario_dir / "scenario.yaml"
        scenario_file.write_text("")

        _load_cascading_tracking_env(str(scenario_file))
    ...
    def test_single_level_below_cwd(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        (tmp_path / ".tracking.env").write_text("LEVEL=root")
        (sub / ".tracking.env").write_text("LEVEL=sub")
