#!/bin/bash
# Run nccl-tests with 1 GPU per node (pure IB, no NVLink contamination).
# --ntasks-per-node=1 --gpus-per-node=1 without --exclusive lets the scheduler
# place 1 GPU per node without blocking the remaining 3 GPUs on each node.
#
# Submit with the desired node count:
#   sbatch --nodes=2  experiments/validate_simccl/run_nccl_1g_jupiter.sh   # 2n1g
#   sbatch --nodes=16 experiments/validate_simccl/run_nccl_1g_jupiter.sh   # 16n1g
#
# Output JSON: results/nccl_<collective>_<N>n1g_jupiter.json
#
#SBATCH --job-name=nccl-1g
#SBATCH --nodes=2
...
echo "Running nccl-tests: ${CONFIG_LABEL} (pure IB, ${NUM_NODES} nodes × ${GPUS_PER_NODE} GPU)"
...
for COLLECTIVE in AllReduce AllGather ReduceScatter AllToAll; do
    ...
    srun "${BIN}" -b "${MIN_BYTES}" -e "${MAX_BYTES}" -f "${STEP_FACTOR}" \
