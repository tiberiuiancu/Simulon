#!/bin/bash
# Run nccl-tests with 2 GPUs on a single node — the exact communicator a TP=2
# group uses (2 GH200s over NVLink). This is the falsification test for the
# TP2/SP=true under-prediction: does a 2-rank intra-node collective have a
# materially different busbw than the 4-rank profile we calibrated on?
#
#   sbatch experiments/validate_simccl/run_nccl_2g_jupiter.sh   # 1n2g
#
# Output JSON: results/nccl_<collective>_1n2g_jupiter.json
#
#SBATCH --job-name=nccl-2g
#SBATCH --nodes=1
...
echo "Running nccl-tests: ${CONFIG_LABEL} (${NUM_NODES} node × ${GPUS_PER_NODE} GPUs over NVLink = TP=2 communicator)"
