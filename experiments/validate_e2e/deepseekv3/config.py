...
def set_deepseek_v3_pipeline_model_parallel_layout(
    ...
    layout_map = {
        ...
        (8, 1): [["embedding"] + ["decoder"] * 8]
        + [["decoder"] * 8] * 6
        + [["decoder"] * 5 + last_layer],
        (4, 2): [["embedding"] + ["decoder"] * 8]
        + [["decoder"] * 8] * 6
        + [["decoder"] * 5 + last_layer],
        (16, 1): [["embedding"] + ["decoder"] * 4]
        + [["decoder"] * 4] * 14
        + [["decoder"] + last_layer],
        (8, 2): [["embedding"] + ["decoder"] * 4]
        + [["decoder"] * 4] * 14
        + [["decoder"] + last_layer],
        (4, 4): [["embedding"] + ["decoder"] * 4]
        + [["decoder"] * 4] * 14
        + [["decoder"] + last_layer],
    }
...
def deepseek_v3_pretrain_config() -> ConfigContainer:
    ...
    # Model config
    cfg.model = AutoBridge.from_hf_pretrained("deepseek-ai/DeepSeek-V3").to_megatron_provider(
        load_weights=False
    )
    ...
    cfg.model.pipeline_model_parallel_layout = (
        None  # Will be set by set_deepseek_v3_pipeline_model_parallel_layout
    )
