...
├── workload.py      # CollectiveWorkload, MegatronWorkload, Training, Parallelism
...
| Add parallelism dimension | `workload.py` → `Parallelism` | Also update trace_tracer rank formula |
...
- **Speed/latency as strings** — `"400Gbps"`, `"0.005ms"` parsed by `network_populate.py:_parse_speed/_parse_latency`
...
- **Speed strings not validated at parse time** — `"400Gbps"` is just a string in Pydantic; parsing happens in `network_populate.py` at simulation time
