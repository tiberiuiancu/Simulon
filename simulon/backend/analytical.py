...
class AnalyticalBackend(Backend):
    ...
    def run_trace(
        self, scenario: ScenarioConfig, _resolved_algorithm: str | None = None
    ) -> ExecutionDAG:
        if isinstance(scenario.workload, MegatronWorkload):
            ...
            raise ValueError(
                f"AnalyticalBackend does not support {type(scenario.workload).__name__}"
            )
    ...
    def simulate(self, scenario: ScenarioConfig) -> tuple[ExecutionDAG, SimulationResult]:
        ...
        if isinstance(scenario.workload, MegatronWorkload):
            ...
            resolved_algorithm, intra_bw_GBps, inter_bw_GBps = cal_busbw(
        ...
        dag = self.run_trace(scenario, _resolved_algorithm=resolved_algorithm)
        logger.info(
